...
class PlanetRemoteMediator(
        ...
        try {
            ...
            database.withTransaction {
                ...
                if (loadType == LoadType.REFRESH) {
                    database.planetRemoteKeysDao().clearRemoteKeys()
                    database.planetDao().clearAll()
                }
                val prevKey = if (page == STARTING_PAGE_INDEX) null else page - 1
                val nextKey = if (endOfPaginationReached) null else page + 1
                val keys = elements.map {
                    PlanetRemoteKeys(id = it.getNumber(), prevKey = prevKey, nextKey = nextKey)
                }
                database.planetRemoteKeysDao().insertAll(keys)
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, Planet>): PlanetRemoteKeys? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.let { planet ->
